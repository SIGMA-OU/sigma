...
int mainKanako(){
	...
	string Training_data_dir = "E:\\Kanako_data\\CELL\\TRAINING_DATA\\";
	string Test_data_dir = "E:\\Kanako_data\\CELL\\TEST_DATA\\";
	...
	for (int i = 0; i < TRAINING_MINIBATCH_NUM; i++){
		...
		training_label_set.push_back(label_temp);
	}
	for (int i = 0; i < TEST_MINIBATCH_NUM; i++){
		string test_file_path = Test_data_dir + "TEST_MINIBATCH_" + to_string(i);
		string label_file_path = Test_data_dir + "TEST_LABEL_MINIBATCH_" + to_string(i);
		Sigma_Matrix test_temp;
		Sigma_Matrix label_temp;
		test_temp.load(test_file_path);
		label_temp.load(label_file_path);
		test_set.push_back(test_temp);
		test_label_set.push_back(label_temp);
	}
	std::cout << "finish making dataset" << endl;

	//モデルの作成
	Input_Layer input;
	Convolution2D_Layer conv1(3, 3, 32, 1, false, relu), conv2(3, 3, 64, 1, false, relu);
	Pooling2D_Layer pool1(2,2,2,false,MAX_POOLING),pool2(2,2,2,false,MAX_POOLING);
	Fully_Connect_Layer full1(1024,relu),full2(1,sig);
	Convolution2D_Layer conv3(3, 3, 64, 1, false, relu), conv4(3, 3, 64, 1, false, relu);
	...
	ofstream delta("E:\\Kanako_data\\CELL\\result\\trial11\\delta.csv", ios::app | ios::out);
	ofstream result("E:\\Kanako_data\\CELL\\result\\trial11\\result.csv", ios::app | ios::out);
	ofstream accuracy("E:\\Kanako_data\\CELL\\result\\trial11\\accuracy.csv", ios::app | ios::out);
	...
	for (int i = 0; i <= TRAINING_NUM; i++){
		...
		if (i % 500 == 0){
			...
			for (int j = 0; j < TEST_MINIBATCH_NUM; j++){
