...
int main1(){
	...
	Input_Layer input;
	Convolution2D_Layer conv1(3, 3, 32, 1, true, relu, 0, 0.1f), conv2(3, 3, 1, 1, true, relu, 0, 0.1f);
	...
	for (int i = 0; i < 3000; i++){
		...
		cout << i <<","<<model.learn(training_data[idx], training_data[idx]) << endl;
	}
	/*opt.learning_rate_ = 0.0001f;
	model.set_optimizer(opt);
	...
	for (int i = 0; i < training_data.size(); i++){
		string file_name =  "C:\\Data_set\\CAE_RESULT\\CAE\\" + to_string(i);
		model.infer(training_data[i]);
		conv1.output().save(file_name);
	}
}